...
gwasfile<-"/home/samman/Documents/publishing/ewas/Analysis/EWAS/BIO1.txt"
traitName<-"BIO1"
ResultFolder<-"/home/samman/Documents/publishing/ewas/Analysis/EWAS/Results/BIO1"
...
gwasData$fdr<-p.adjust(gwasData$pvalues, method = "BH")
# calculate the threshold
threshold<-quantile(gwasData$pvalues, probs = qthreshold, na.rm = TRUE)

# select the log global value
log.global<- NA

loggvalues<-gwasData[gwasData$pvalues<threshold,]$pvalues
...
logSignvalues<-gwasData[gwasData$fdr<fdrcutoff,]$pvalues
log.Sign<-NA
...
gwasData<-gwasData[order(gwasData$pvalues),]
...
if (nrow(gwasData[gwasData$pvalues<threshold,])>0) {
  # select the significant SNPs according to the fdr
  write.table(gwasData[gwasData$pvalues<threshold,], file = paste(ResultFolder,"/",traitName,"_threshold.tsv",sep=""), sep = "\t", quote = FALSE, row.names = FALSE)
}
...
#CA1	5825982	322900	8	A	G	0.051	-9.933371e-01	1.231881e+00	-6.014125e+02	1.000000e-05	
# pvalues  fscores adj.r.squared chr       pos id ref alt qual filter info   format fdr

# add rs using chr and pos
gwasData$rs<-paste(gwasData$chr,gwasData$pos,sep=":")
# add ps using pos
gwasData$ps<-gwasData$pos
# p_wald is the pvlaues
gwasData$p_wald<-gwasData$pvalues
